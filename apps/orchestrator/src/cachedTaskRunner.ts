...
function buildCachedTaskResponse(
  ...
  const checkpoint = cachedRecord.response.checkpoint ?? createTaskCheckpoint({
    ...
    verifierResult: cachedRecord.response.verifierResult,
    output: cachedRecord.response.output,
    providerResult: cachedRecord.response.providerResult,
    costEstimate: cachedRecord.response.costEstimate,
    pipeline,
    trace,
...
function annotateLiveResponse(
  ...
  const checkpoint = taskResponse.checkpoint ?? createTaskCheckpoint({
    ...
    diagnostic: taskResponse.diagnostic,
